...
-- drop TABLE relations_offspring;
CREATE TABLE relations_offspring (
 id INTEGER PRIMARY KEY,
 idRelation INTEGER NOT NULL,
 idUser INTEGER NOT NULL,
 FOREIGN KEY (idRelation) REFERENCES relations(id),
...
INSERT into users (name,birthdayLocationId) values ('child1',1);
INSERT into users (name,birthdayLocationId) values ('child2',1);

INSERT into relations_offspring (idRelation,idUser) values (1,3);
INSERT into relations_offspring (idRelation,idUser) values (1,4);
select * from relations_offspring;
...
WHERE (u.id != r.idRelA and u.id != r.idRelB)
    ...
    select rOff.*,u.name
    from relations_offspring rOff, users u , relations rel
    WHERE u.id = rOff.idUser
    and rel.id = rOff.idRelation
    and rOff.idRelation= rOff.idRelation = 1;
